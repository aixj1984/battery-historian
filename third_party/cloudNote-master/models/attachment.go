...
func GetTemplate() (v []Attachment, err error) {
	o := orm.NewOrm()
	var ch []Attachment
	num, err := o.QueryTable(new(Attachment)).Filter("type", "template").All(&ch)
	if num > 0 && err == nil {
		return ch, nil
	}
	return nil, err
}
func GetPaper() (v []Attachment, err error) {
	o := orm.NewOrm()
	var ch []Attachment
	num, err := o.QueryTable(new(Attachment)).Filter("type", "paper").All(&ch)
	if num > 0 && err == nil {
		return ch, nil
	}
	return nil, err
